...
// جزء من الـ IIFE المشترك (لا import/export). الترتيب محفوظ في js/_js_order.json
  ...
  function renderMutabaanaBody(items){
    ...
    const who = S.mtbWho || "mine";   // الفلتر يطبّق على متابعة مين
    ...
    const whoChip = (val,lbl)=>{ const a=who===val; return `<button data-act="mtbWho" data-w="${val}" style="width:auto;padding:5px 12px;border-radius:99px;border:1px solid ${a?'transparent':'var(--line)'};background:${a?'var(--brand-gold,#c9a14a)':'var(--surface)'};color:${a?'#1e3a2f':'var(--ink)'};font:inherit;font-size:12px;font-weight:800;cursor:pointer">${lbl}</button>`; };
    html += `<div style="display:flex;gap:6px;flex-wrap:wrap;margin:14px 2px 4px;align-items:center"><span class="muted" style="font-size:12px;flex:none">حسب متابعة:</span>${whoChip("mine","أنا")}${whoChip("partner",esc(_cn.partner))}${whoChip("either","كلانا")}</div>`;
    html += `<div style="display:flex;gap:8px;flex-wrap:wrap;margin:6px 2px 2px">${chip("all","الكل")}${chip("not_started","لم يتم البدء")}${chip("in_progress","جاري المشاهدة")}${chip("completed","تمت المشاهدة")}</div>`;
    const matchF = (r)=>{ if(F==="all") return true; const p=r.prog||{}; const m=p.mine||"not_started", pa=p.partner||"not_started"; return who==="partner"? pa===F : who==="either"? (m===F||pa===F) : m===F; };
    const q=(S.mtbQ||"").trim().toLowerCase();
    const hit=(s)=>String(s||"").toLowerCase().includes(q);
    const shown = items.filter(r=> matchF(r) && (!q || hit(r.title)||hit(r.speaker)) );
  ...
  function renderMutabaana(){
    ...
    el.innerHTML = pageTitle("متابعتنا","شوف إنت و"+coupleNames().partner+" وصلتوا لفين في كل مرحلة — والتحديثات بتظهر تلقائيًا كل شوية 🌿")
      + `<div class="card" style="padding:10px 12px;margin-bottom:12px"><input id="mtbSearch" type="search" placeholder="دوّر على مادة بالاسم داخل المتابعة…" value="${esc(S.mtbQ||"")}"></div>`
      + `<div id="mtbBody"><div class="empty">…تحميل</div></div>`;
    _mtbSig = "";
    const si=document.getElementById("mtbSearch");
    if(si){ si.addEventListener("input",()=>{ S.mtbQ=si.value; if(_mtbItems) renderMutabaanaBody(_mtbItems); }); }
    loadMutabaana();
