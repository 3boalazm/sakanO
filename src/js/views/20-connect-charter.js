      ...
      document.getElementById("cBody").innerHTML =
        ...
        <div class="card"><div class="eyebrow">رسالة للمستقبل</div>
          ...
          ${caps.length? caps.map(c=>`<div class="card tight" style="margin-top:8px">
            ...
            ${c.mine?`<div class="actions" style="margin-top:8px;flex-wrap:wrap;gap:6px">
              <button class="linkbtn" data-act="delCapsule" data-id="${esc(c.id)}">حذف</button>
              ${c.canOpen?`<button class="linkbtn" data-act="convCapsule" data-id="${esc(c.id)}" data-to="${c.manual?'date':'manual'}">${c.manual?'حوّلها لفتح بتاريخ':'خلّيها مختومة (أبعتها بنفسي)'}</button>
