...
const BACKUP_COLLECTIONS = [
  ...
  'notes', 'charter', 'tasks', 'budget', 'shopping',
...
// ---- يوم الخطوبة (engagement day checklist — قابلة للتعديل، مقسّمة لصفحات فرعية) ----
async function seedEngagementSection(s, sectionId) {
  ...
  for (const text of def.items) {
    ...
    const id = uid();
    await C('engagement').doc(id).set({ pairId: s.pairId, section: sectionId, text, state: 'pending', seed: true, order: n, createdBy: null, createdAt: now + n });
    n++;
  }
}
export async function listEngagement(s, sectionId) {
  if (!ENGAGEMENT_SECTION_IDS.includes(sectionId)) throw new Err(404, 'NOT_FOUND');
  await seedEngagementSection(s, sectionId);
  const snap = await C('engagement').where('pairId', '==', s.pairId).where('section', '==', sectionId).get();
  return snap.docs.map((d) => ({ id: d.id, ...d.data() })).sort((a, b) => (a.order ?? 9999) - (b.order ?? 9999) || a.createdAt - b.createdAt);
}
export async function createEngagementItem(s, sectionId, text) {
  if (!ENGAGEMENT_SECTION_IDS.includes(sectionId)) throw new Err(404, 'NOT_FOUND');
  const t = (text || '').toString().trim().slice(0, 300);
  if (!t) throw new Err(400, 'BAD_TEXT');
  const id = uid();
  await C('engagement').doc(id).set({ pairId: s.pairId, section: sectionId, text: t, state: 'pending', seed: false, order: 9999, createdBy: s.userId, createdAt: Date.now() });
...
export async function editEngagementItem(s, id, text) {
  const t = (text || '').toString().trim().slice(0, 300);
  if (!t) throw new Err(400, 'BAD_TEXT');
  const ref = await ownedDoc('engagement', id, s);
  await ref.update({ text: t });
  return { id, ...(await ref.get()).data() };
}
