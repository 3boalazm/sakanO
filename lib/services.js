...
export async function listMessages(s) {
  ...
  return snap.docs
    .map((d) => { const x = d.data(); return { id: d.id, text: x.text, mine: x.userId === s.userId, createdAt: x.createdAt }; })
...
export async function sendMessage(s, text) {
  ...
  const id = uid();
  await C('messages').doc(id).set({ pairId: s.pairId, userId: s.userId, text: t, createdAt: Date.now() });
  return { id, text: t, mine: true, createdAt: Date.now() };
}
