...
import { ENGAGEMENT_SECTIONS, ENGAGEMENT_SECTION_IDS } from './engagement-content.js';
...
// إعادة توليد كود الإقران (طوارئ): تفتح مقعد الشريك من جديد بكود لمرّة واحدة،
// وتُبطل وصول حساب الشريك القديم بالكامل (جلسات/إيميل/مستخدم). للمُنشئ (userA) فقط.
async function revokeUser(userId) {
  try { const ss = await C('sessions').where('userId', '==', userId).get(); for (const d of ss.docs) await C('sessions').doc(d.id).delete(); } catch (_) {}
  try { const ei = await C('emailIndex').where('userId', '==', userId).get(); for (const d of ei.docs) await C('emailIndex').doc(d.id).delete(); } catch (_) {}
  try { await C('users').doc(userId).delete(); } catch (_) {}
}
export async function regeneratePairCode(s) {
  const pairRef = C('pairs').doc(s.pairId);
  const pair = (await pairRef.get()).data();
  if (!pair) throw new Err(404, 'NOT_FOUND');
  if (pair.userA !== s.userId) throw new Err(403, 'ONLY_CREATOR');
  if (pair.userB) await revokeUser(pair.userB);          // اقطع وصول الشريك القديم نهائيًا
  if (pair.code) { try { await C('pairCodes').doc(pair.code).delete(); } catch (_) {} }
  const code = code8();
  await pairRef.update({ userB: null, status: 'pending', code });
  await C('pairCodes').doc(code).set({ pairId: s.pairId });
  await record(s.pairId, 'pair', s.pairId, 'pair_recoded', { actor: s.userId });
  return { ok: true, pairCode: code };
}
// ============================================================
// نسخة احتياطية / استرجاع — تصدير كل بيانات الميثاق كـ JSON، واستعادتها في حساب جديد.
// نتجاهل بيانات الهوية (الحسابات/الجلسات) والبيانات العابرة (الحضور/القراءة) ومفاتيح منع التكرار.
const BACKUP_COLLECTIONS = [
  'resources', 'progress', 'summaries', 'questions', 'responses',
  'decisions', 'decisionQuestions', 'decisionConfirmations',
  'notes', 'charter', 'tasks', 'budget', 'shopping', 'engagement',
  'messages', 'resChat', 'focus', 'mood',
  'keys', 'gratitude', 'wishes', 'safespace', 'capsules', 'events',
];
export async function exportBackup(s) {
  const pair = (await C('pairs').doc(s.pairId).get()).data() || {};
  const out = { app: 'sakan', version: 1, exportedAt: Date.now(), pairId: s.pairId, userA: pair.userA || null, userB: pair.userB || null, me: s.userId, collections: {} };
  for (const c of BACKUP_COLLECTIONS) {
    const snap = await C(c).where('pairId', '==', s.pairId).get();
    out.collections[c] = snap.docs.map((d) => ({ id: d.id, data: d.data() }));
  }
  return out;
}
export async function restoreBackup(s, backup) {
  if (!backup || backup.app !== 'sakan' || !backup.collections) throw new Err(400, 'BAD_BACKUP');
  const newPair = s.pairId;
  const oldPair = backup.pairId;
  const pair = (await C('pairs').doc(newPair).get()).data() || {};
  // ربط مُعرّفات المستخدمين القديمة بالجديدة بالترتيب (المنشئ↔المنشئ، الشريك↔الشريك)
  const umap = {};
  if (backup.userA && pair.userA) umap[backup.userA] = pair.userA;
  if (backup.userB && pair.userB) umap[backup.userB] = pair.userB;
  if (backup.me && !umap[backup.me]) umap[backup.me] = s.userId; // ضمان أن "أنا" أرجع لنفسي
  const remapId = (id) => {
    if (id === oldPair) return newPair;
    const i = id.indexOf('_'); if (i < 0) return id;
    const head = id.slice(0, i), tail = id.slice(i + 1);
    if (head === oldPair) return newPair + '_' + (umap[tail] || tail); // pairId_userId (mood)
    if (tail === 'shared') return id;                                  // resourceId_shared (notes)
    return umap[tail] ? head + '_' + umap[tail] : id;                  // x_userId
  };
  const USER_FIELDS = ['userId', 'actor', 'createdBy', 'setBy'];
  const remapData = (d) => { const o = { ...(d || {}) }; if ('pairId' in o) o.pairId = newPair; for (const k of USER_FIELDS) { if (o[k] != null && umap[o[k]]) o[k] = umap[o[k]]; } return o; };
  let count = 0;
  for (const c of BACKUP_COLLECTIONS) {
    const docs = backup.collections[c]; if (!Array.isArray(docs)) continue;
    for (const doc of docs) {
      if (!doc || !doc.id) continue;
      await C(c).doc(remapId(doc.id)).set(remapData(doc.data));
      count++;
    }
  }
  await record(newPair, 'pair', newPair, 'restored', { actor: s.userId, meta: { count, from: oldPair || null } });
...
export async function listMyJourney(s, limit = 80) {
  // رحلتنا ببانلين: نشاطي أنا (mine) + نشاط شريكي (partner)، كلٌّ في عموده.
  // كله أحداث type === 'action' (المُسجّلة وقت كل طلب تعديلي ناجح).
  ...
  const shape = (d) => {
    ...
    return { actor: e.actor, id: d.id, entity: 'action', method: (e.meta && e.meta.method) || '', path, title, createdAt: e.createdAt };
  };

  const all = snap.docs.map(shape).filter(Boolean);
  const pick = (uid) => all.filter((e) => e.actor === uid).sort((a, b) => b.createdAt - a.createdAt).slice(0, limit);
  const mine = pick(s.userId).map(({ actor, ...rest }) => rest);
  const partner = pid ? pick(pid).map(({ actor, ...rest }) => rest) : [];
  return { mine, partner, myId: s.userId, partnerId: pid || null };
}
...
// نموذج المهمة: { title, details, owner, due, done, steps: [{id,text,done}] }
// لو فيها مراحل (steps) → نسبة الإنجاز تتحسب أوتوماتيك و done مشتقّة من اكتمال كل المراحل.
// لو من غير مراحل → done عبارة عن زرّ يدوي واحد (السلوك القديم).
function shapeTask(id, x) {
  const steps = Array.isArray(x.steps) ? x.steps : [];
  const stepsTotal = steps.length;
  const stepsDone = steps.filter((st) => st.done).length;
  const done = stepsTotal > 0 ? stepsDone === stepsTotal : !!x.done;
  const progress = stepsTotal > 0 ? Math.round((stepsDone / stepsTotal) * 100) : (done ? 100 : 0);
  return { id, title: x.title, details: x.details || '', owner: x.owner, due: x.due || null, steps, stepsTotal, stepsDone, progress, done, createdAt: x.createdAt };
}
export async function listTasks(s) {
  const snap = await C('tasks').where('pairId', '==', s.pairId).get();
  return snap.docs.map((d) => shapeTask(d.id, d.data())).sort((a, b) => a.createdAt - b.createdAt);
}
export async function createTask(s, b) {
  const id = uid();
  const owner = ['m', 'd', 'both'].includes(b.owner) ? b.owner : 'both';
  const steps = (Array.isArray(b.steps) ? b.steps : [])
    .map((t) => (t || '').toString().trim()).filter(Boolean).slice(0, 30)
    .map((text) => ({ id: uid(), text: text.slice(0, 200), done: false }));
  await C('tasks').doc(id).set({ pairId: s.pairId, title: (b.title || 'مهمة').slice(0, 300), details: (b.details || '').toString().slice(0, 1000), owner, due: b.due || null, done: false, steps, createdBy: s.userId, createdAt: Date.now() });
  await record(s.pairId, 'task', id, 'created', { actor: s.userId });
  return shapeTask(id, (await C('tasks').doc(id).get()).data());
}
export async function toggleTask(s, id) {
  const ref = await ownedDoc('tasks', id, s);
  const x = (await ref.get()).data();
  // لو فيها مراحل، الزرّ بيخلّص/يفرّغ كل المراحل عشان يفضل done مشتقّ
  if (Array.isArray(x.steps) && x.steps.length) {
    const allDone = x.steps.every((st) => st.done);
    const steps = x.steps.map((st) => ({ ...st, done: !allDone }));
    await ref.update({ steps, done: !allDone });
  } else {
    await ref.update({ done: !x.done });
  }
  return shapeTask(id, (await ref.get()).data());
}
export async function deleteTask(s, id) { const ref = await ownedDoc('tasks', id, s); await ref.delete(); return { ok: true }; }
export async function addTaskStep(s, id, text) {
  const ref = await ownedDoc('tasks', id, s);
  const t = (text || '').toString().trim().slice(0, 200);
  if (!t) throw new Err(400, 'EMPTY_STEP');
  const x = (await ref.get()).data();
  const steps = (Array.isArray(x.steps) ? x.steps : []).slice(0, 50);
  steps.push({ id: uid(), text: t, done: false });
  await ref.update({ steps });
  return shapeTask(id, (await ref.get()).data());
}
export async function toggleTaskStep(s, id, stepId) {
  const ref = await ownedDoc('tasks', id, s);
  const x = (await ref.get()).data();
  const steps = (Array.isArray(x.steps) ? x.steps : []).map((st) => st.id === stepId ? { ...st, done: !st.done } : st);
  const done = steps.length > 0 && steps.every((st) => st.done);
  await ref.update({ steps, done });
  return shapeTask(id, (await ref.get()).data());
}
export async function deleteTaskStep(s, id, stepId) {
  const ref = await ownedDoc('tasks', id, s);
  const x = (await ref.get()).data();
  const steps = (Array.isArray(x.steps) ? x.steps : []).filter((st) => st.id !== stepId);
  const done = steps.length > 0 && steps.every((st) => st.done);
  await ref.update({ steps, done });
  return shapeTask(id, (await ref.get()).data());
}
...
// ---- يوم الخطوبة (engagement day checklist — صفحة واحدة، بمواضيع مطويّة + فلو زمني موحّد) ----
async function seedEngagementSection(s, sectionId) {
  const def = ENGAGEMENT_SECTIONS.find((x) => x.id === sectionId);
  if (!def) return;
  const existing = await C('engagement').where('pairId', '==', s.pairId).where('section', '==', sectionId).where('seed', '==', true).get();
  const have = new Set(existing.docs.map((d) => d.data().text));
  const now = Date.now(); let n = 0;
  for (const it of def.items) {
    if (have.has(it.text)) { n++; continue; }
    const id = uid();
    const owner = ['m', 'd', 'both'].includes(it.owner) ? it.owner : 'both';
    await C('engagement').doc(id).set({
      pairId: s.pairId, section: sectionId, text: it.text, owner, state: 'pending',
      seed: true, flowOrder: def.flowBase + n * 10, createdBy: null, createdAt: now + n,
    });
    n++;
  }
}
async function seedAllEngagement(s) {
  for (const def of ENGAGEMENT_SECTIONS) await seedEngagementSection(s, def.id);
}
// كل العناصر في كل الأقسام — دفعة واحدة (تستخدمها الصفحة الموحّدة لعرضين: بالمواضيع، وبالفلو الزمني)
export async function listEngagementAll(s) {
  await seedAllEngagement(s);
  const snap = await C('engagement').where('pairId', '==', s.pairId).get();
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
export async function createEngagementItem(s, sectionId, text, owner) {
  if (!ENGAGEMENT_SECTION_IDS.includes(sectionId)) throw new Err(404, 'NOT_FOUND');
  const t = (text || '').toString().trim().slice(0, 300);
  if (!t) throw new Err(400, 'BAD_TEXT');
  const ow = ['m', 'd', 'both'].includes(owner) ? owner : 'both';
  const def = ENGAGEMENT_SECTIONS.find((x) => x.id === sectionId);
  const id = uid();
  // العنصر المُضاف يدويًا بيتحط آخر قسمه في الفلو (بعد كل عناصر البذرة، وقبل القسم اللي بعده بمساحة كبيرة)
  await C('engagement').doc(id).set({ pairId: s.pairId, section: sectionId, text: t, owner: ow, state: 'pending', seed: false, flowOrder: (def ? def.flowBase : 0) + 500, createdBy: s.userId, createdAt: Date.now() });
  await record(s.pairId, 'engagement', id, 'created', { actor: s.userId });
  return { id, ...(await C('engagement').doc(id).get()).data() };
}
export async function setEngagementState(s, id, state) {
  if (!['pending', 'done', 'skip'].includes(state)) throw new Err(400, 'BAD_STATE');
  const ref = await ownedDoc('engagement', id, s);
  await ref.update({ state });
  return { id, ...(await ref.get()).data() };
}
export async function editEngagementItem(s, id, text, owner) {
  const ref = await ownedDoc('engagement', id, s);
  const patch = {};
  if (text != null) { const t = text.toString().trim().slice(0, 300); if (!t) throw new Err(400, 'BAD_TEXT'); patch.text = t; }
  if (owner != null && ['m', 'd', 'both'].includes(owner)) patch.owner = owner;
  if (Object.keys(patch).length) await ref.update(patch);
  return { id, ...(await ref.get()).data() };
}
export async function deleteEngagementItem(s, id) { const ref = await ownedDoc('engagement', id, s); await ref.delete(); return { ok: true }; }
...
const CURRICULUM = [
  ...
  // ---- المرحلة 0: التأصيل ----
  { type:'video',  stage:0, order:201, priority:'medium', title:'الزواج عبودية العمر — دورة أمومة واعية', speaker:'الشيخ محمد خيري', link:'https://youtu.be/J2OZgA5lHeE', purpose:'ترسيخ المفهوم الروحي والعبادي للزواج.' },
  // ---- المرحلة 1: الاختيار ----
  { type:'video',  stage:1, order:202, priority:'medium', title:'اختيار شريك الحياة: بين المروءة والعقل الحكيم', speaker:'د. أحمد العربي', link:'https://youtu.be/SRsXB-EcvXk', purpose:'معايير وأسس الاختيار الشرعية والعملية.' },
  { type:'video',  stage:1, order:203, priority:'medium', title:'فن اختيار شريك الحياة — م. علاء حامد', speaker:'م. علاء حامد', link:'https://youtu.be/P6quAlK1a9I', purpose:'كلام عملي وواقعي حول عملية الاختيار.' },
  { type:'video',  stage:1, order:204, priority:'medium', title:'المرأة الذكية ومفاتيح الرجل', speaker:'الشيخ أبو إسحاق الحويني', link:'https://youtu.be/TuUEWKwwSH0', purpose:'فهم سيكولوجية الرجل والتعامل الذكي مع طبيعته.' },
  // ---- المرحلة 1: قضايا معاصرة ----
  { type:'video',  stage:1, order:205, priority:'medium', title:'القوامة والنسوية بين الفطرة وتحديات العصر (ج١)', speaker:'د. الحارث المزيدي', link:'https://youtu.be/gla27IuF5zM', purpose:'فهم مفهوم القوامة في ضوء التحديات المعاصرة.' },
  { type:'video',  stage:1, order:206, priority:'medium', title:'القوامة والنسوية بين الفطرة وتحديات العصر (ج٢)', speaker:'د. الحارث المزيدي', link:'https://youtu.be/GYi_DS_5yHs', purpose:'استكمال النقاش حول الفطرة والتحديات المعاصرة.' },
  // ---- المرحلة 1: نقاش جماعي ----
  { type:'video',  stage:1, order:207, priority:'medium', title:'إيه المشكلة في الزواج؟', speaker:'ياسر ممدوح / محمد الغليظ / أمير منير', link:'https://youtu.be/XqDe0Wgdt6M', purpose:'نقاش متعدد الزوايا حول إشكاليات الزواج المعاصر.' },
  // ---- المرحلة 3: بناء الزواج ----
  { type:'video',  stage:3, order:208, priority:'medium', title:'كيف نبني زواجاً مستقراً', speaker:'د. جاسم محمد المطوع', link:'https://youtu.be/_57DQt0CP48', purpose:'الأسس العملية لبناء زواج مستقر ودائم.' },
  // ---- القوائم (playlists) ----
  { type:'course', stage:4, order:209, priority:'medium', title:'سلسلة البيت المسلم', speaker:'م. أيمن عبد الرحيم', link:'https://youtube.com/playlist?list=PLnFJTGgdQYTOWcUXdFLzpsgy6_rlXkwrh', purpose:'سلسلة شاملة لبناء الأسرة المسلمة.' },
  ...
  { type:'course', stage:4, order:212, priority:'medium', title:'الدورة التأهيلية — ألف باء الزواج', speaker:'د. عبد الرحمن الهاشمي', link:'https://youtube.com/playlist?list=PL1_wZ0MJhhg4lgNinwzQ5SA_bctoA1HxE', purpose:'دورة تأهيلية متعمّقة للعلاقة الزوجية.' },
  ...
  // ---- المرحلة 5: الاستمرار والتطوير ----
  { type:'video',  stage:5, order:214, priority:'later',  title:'سيرته ﷺ لتصحيح مسارك — أمومة واعية', speaker:'الشيخ محمد خيري', link:'https://youtu.be/oLRKrPRzCWg', purpose:'الاستضاءة بسيرة النبي ﷺ لتصحيح مسار الحياة الزوجية.' },
  { type:'video',  stage:5, order:215, priority:'later',  title:'حتى ترضى — سلسلة أمومة واعية', speaker:'الشيخ محمد خيري', link:'https://youtu.be/DHVhe0yWQ6E', purpose:'مفاهيم الرضا والقناعة في العلاقة الزوجية.' },
  { type:'video',  stage:5, order:216, priority:'later',  title:'قواعد حياتية للأسرة الرضية — أمومة واعية', speaker:'الشيخ محمد خيري', link:'https://youtu.be/qkNix3ol8Jo', purpose:'قواعد عملية لحياة أسرية سعيدة.' },
  { type:'video',  stage:5, order:217, priority:'later',  title:'الحياء المفتقد — سلسلة الأدب', speaker:'د. أحمد العربي', link:'https://youtu.be/PBGXnDYnkTI', purpose:'أهمية الحياء في العلاقة الزوجية.' },
];
...
// موضوع نقاش مستقل (مش مربوط بمورد) — يمشي على نفس فلو الإجابة/الكشف بالضبط (مفتاحه questionId).
export async function addStandaloneQuestion(s, text) {
  const t = String(text || '').trim();
  if (!t) throw new Err(400, 'EMPTY');
  const id = uid(), now = Date.now();
  await C('questions').doc(id).set({ pairId: s.pairId, resourceId: null, standalone: true, text: t.slice(0, 500), state: 'open', responseCount: 0, order: now, createdAt: now });
  await record(s.pairId, 'question', id, 'created', { actor: s.userId, meta: { standalone: true } });
  return { id, ...(await C('questions').doc(id).get()).data() };
}
...
// ---- رسالة للمستقبل ----
// نمطان: (1) مؤقتة بتاريخ openDate → تتفتح للشريك تلقائيًا بعد التاريخ.
//        (2) يدوية manualSeal → تفضل مختومة لحد ما صاحبها يفتحها/يبعتها بنفسه.
export async function listCapsules(s) {
  const now = Date.now();
  const snap = await C('capsules').where('pairId', '==', s.pairId).get();
  return snap.docs.map((d) => {
    const x = d.data(); const mine = x.userId === s.userId;
    const dateReached = x.openAt && now >= x.openAt;
    const opened = x.opened === true || dateReached || (!x.openAt && !x.manualSeal);
    const visible = opened; // مختومة = مقفولة على الاتنين، حتى صاحبها، لحد ما تتفتح
    const canOpen = mine && !opened; // صاحبها يقدر يفتحها/يبعتها دلوقتي
    return { id: d.id, mine, manual: !!x.manualSeal, opened: !!opened, canOpen, openDate: x.openDate || null, sealed: !visible, content: visible ? x.content : null, createdAt: x.createdAt };
  }).sort((a, b) => b.createdAt - a.createdAt);
}
export async function createCapsule(s, b) {
  const id = uid();
  const manualSeal = !!b.manualSeal;
  const parsed = b.openDate ? Date.parse(b.openDate) : NaN;
  if (!manualSeal && b.openDate && isNaN(parsed)) throw new Err(400, 'BAD_DATE'); // كان بيتحفظ كأنه اتفتح فورًا من غير أي تنبيه
  await C('capsules').doc(id).set({ pairId: s.pairId, userId: s.userId, content: (b.content || '').slice(0, 3000), openDate: manualSeal ? null : (b.openDate || null), openAt: (manualSeal || isNaN(parsed)) ? null : parsed, manualSeal, opened: false, createdAt: Date.now() });
  return { ok: true };
}
export async function openCapsule(s, id) {
  const ref = C('capsules').doc(String(id));
  const snap = await ref.get();
  if (!snap.exists || snap.data().pairId !== s.pairId) throw new Err(404, 'NOT_FOUND');
  if (snap.data().userId !== s.userId) throw new Err(403, 'FORBIDDEN'); // بس صاحبها يفتحها
  await ref.update({ opened: true, openedAt: Date.now() });
  return { ok: true };
}
// حذف رسالة مؤجّلة — صاحبها فقط، وما دامت لسه مختومة (متبعتش ولا وصل تاريخها).
export async function deleteCapsule(s, id) {
  const ref = C('capsules').doc(String(id));
  const snap = await ref.get();
  if (!snap.exists || snap.data().pairId !== s.pairId) throw new Err(404, 'NOT_FOUND');
  if (snap.data().userId !== s.userId) throw new Err(403, 'FORBIDDEN');
...
// تحويل طريقة الفتح: يدوي ↔ بتاريخ — صاحبها فقط، وما دامت لسه مختومة من الطرف الآخر.
export async function convertCapsule(s, id, b) {
  const ref = C('capsules').doc(String(id));
  const snap = await ref.get();
  if (!snap.exists || snap.data().pairId !== s.pairId) throw new Err(404, 'NOT_FOUND');
  const x = snap.data();
  if (x.userId !== s.userId) throw new Err(403, 'FORBIDDEN');
  const dateReached = x.openAt && Date.now() >= x.openAt;
  if (x.opened === true || dateReached) throw new Err(409, 'ALREADY_OPEN'); // اتبعتت/اتفتحت بالفعل
  if (b && b.manualSeal) {
    await ref.update({ manualSeal: true, openDate: null, openAt: null });
  } else {
    const parsed = (b && b.openDate) ? Date.parse(b.openDate) : NaN;
    if (isNaN(parsed)) throw new Err(400, 'BAD_DATE');
    await ref.update({ manualSeal: false, openDate: b.openDate, openAt: parsed });
  }
  return { ok: true, id };
}
...
// إشعارات موحّدة: كل تحديثات الطرف الآخر المشتركة (نشاط الصفحات + دردشات الفيديو + الشات العام).
// الجرس (count) يتصفّر عند فتح لوحة الإشعارات؛ عدّاد الشات (chatUnread) يتصفّر عند فتح الشات.
export async function listUpdates(s, limit = 30) {
  const pid = await partnerId(s);
  if (!pid) return { count: 0, chatUnread: 0, items: [] };
  const nr = (await C('notifReads').doc(s.pairId).get()).data() || {};
  const marker = nr[s.userId] || 0;
  const rd = (await C('reads').doc(s.pairId).get()).data() || {};
  const chatRead = rd[s.userId] || 0;

  const resSnap = await C('resources').where('pairId', '==', s.pairId).get();
  const resMap = {}; resSnap.docs.forEach((d) => { resMap[d.id] = d.data().title || '—'; });

  // 1) نشاط الطرف الآخر على الصفحات (أحداث 'action' لها مسار) بعد علامة الجرس — ما عدا الشات العام
  const SKIP = new Set(['messages', 'presence', 'restore', 'updates']);
  const evSnap = await C('events').where('pairId', '==', s.pairId).get();
  let items = evSnap.docs.map((d) => {
    const e = d.data();
    if (e.actor !== pid || e.type !== 'action' || e.createdAt <= marker) return null;
    const path = (e.meta && e.meta.path) || [];
    if (!path[0] || SKIP.has(path[0])) return null;
    const resourceId = (path[0] === 'resources' && path[1]) ? path[1] : null;
    return { kind: 'update', path, resourceId, title: resourceId ? (resMap[resourceId] || null) : null, createdAt: e.createdAt };
  }).filter(Boolean);

  // 2) دردشات الفيديو (resChat) من الطرف الآخر بعد علامة الجرس
  const rcSnap = await C('resChat').where('pairId', '==', s.pairId).get();
  rcSnap.docs.map((d) => d.data()).filter((x) => x.userId === pid && !x.deleted && x.createdAt > marker)
    .forEach((x) => items.push({ kind: 'resChat', resourceId: x.resourceId, title: resMap[x.resourceId] || 'حلقة', text: (x.text || '').slice(0, 80), createdAt: x.createdAt }));

  // 3) رسائل الشات العام من الطرف الآخر (للجرس بعد علامة الجرس، وعدّاد الشات بعد علامة القراءة)
  const msgSnap = await C('messages').where('pairId', '==', s.pairId).get();
  const partnerMsgs = msgSnap.docs.map((d) => d.data()).filter((x) => x.userId === pid && !x.deleted);
  const chatUnread = partnerMsgs.filter((x) => x.createdAt > chatRead).length;
  partnerMsgs.filter((x) => x.createdAt > marker)
    .forEach((x) => items.push({ kind: 'chat', text: (x.text || '').slice(0, 80), createdAt: x.createdAt }));

  items.sort((a, b) => b.createdAt - a.createdAt);
  if (items.length > limit) items = items.slice(0, limit);
  return { count: items.length, chatUnread, items };
}
export async function markUpdatesRead(s) {
  const ref = C('notifReads').doc(s.pairId);
  const cur = (await ref.get()).data() || {};
  cur[s.userId] = Date.now();
  await ref.set(cur);
  return { ok: true };
