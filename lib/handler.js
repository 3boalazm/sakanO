...
async function route(m, p, body, token) {
  ...
  // ---- يوم الخطوبة (صفحة واحدة: كل الأقسام + فلو زمني موحّد) ----
  if (m === 'GET' && eq(p, ['engagement'])) return S.listEngagementAll(s);
  if (m === 'POST' && p.length === 2 && p[0] === 'engagement') return S.createEngagementItem(s, p[1], body.text, body.owner);
  if (m === 'PUT' && p.length === 3 && p[0] === 'engagement' && p[2] === 'state') return S.setEngagementState(s, p[1], body.state);
  if (m === 'PUT' && p.length === 2 && p[0] === 'engagement') return S.editEngagementItem(s, p[1], body.text, body.owner);
  if (m === 'POST' && p.length === 3 && p[0] === 'engagement' && p[2] === 'delete') return S.deleteEngagementItem(s, p[1]);
