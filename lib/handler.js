...
async function route(m, p, body, token) {
  ...
  // ---- يوم الخطوبة (checklist قابل للتعديل، صفحة مستقلة لكل موضوع) ----
  if (m === 'GET' && p.length === 2 && p[0] === 'engagement') return S.listEngagement(s, p[1]);
  if (m === 'POST' && p.length === 2 && p[0] === 'engagement') return S.createEngagementItem(s, p[1], body.text);
  if (m === 'PUT' && p.length === 3 && p[0] === 'engagement' && p[2] === 'state') return S.setEngagementState(s, p[1], body.state);
  if (m === 'PUT' && p.length === 2 && p[0] === 'engagement') return S.editEngagementItem(s, p[1], body.text);
